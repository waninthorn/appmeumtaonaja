...
@api_view(['POST'])
def post_expend(request):
    if request.method == 'POST':
        serializer = ExpendSerializer(data=request.data)
...
@api_view(['GET'])
def all_expend(request):
    all_expend = Expend.objects.all()
    serializer= ExpendSerializer(all_expend,many=True)
    return Response(serializer.data, status=status.HTTP_200_OK)

@api_view(['DELETE'])
def delete_expend(request,TID):
    exp = Expend.objects.get(id=TID)
...
@api_view(['PUT'])
def update_expend(request, TID):
    exp = Expend.objects.get(id = TID)
    if request.method == 'PUT':
        data = {}
        serializer =ExpendSerializer(exp, data=request.data)
        if serializer.is_valid():
            serializer.save()
            data['status'] = 'updated'
